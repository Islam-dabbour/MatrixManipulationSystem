...
struct Matrix addNewRow(struct Matrix matrix){
    size_t newSize = (size_t)(matrix.rows + 1) * matrix.columns;
    int *expandedArray = realloc(matrix.arr, newSize * sizeof *matrix.arr);
    ...
    matrix.arr = expandedArray;
    for (int column = 0; column < matrix.columns; column++) {
        matrix.arr[matrix.rows * matrix.columns + column] = (rand() % 100) + 1;
    }
    matrix.rows++;
...
int main(){
    ...
    int transposedMultiplicationPipe[2];
    ...
    pipe(transposedMultiplicationPipe);
    ...
    if( pid1 == 0 ){
        ...
        gettimeofday(&endM, NULL); 
          ...
          // This is task4 part b --> transpose the multipled matrix then add a new row with random numbers 

          struct Matrix transposedMultipicationMatrix = matrixTransposition(multipliedMatrix);
            transposedMultipicationMatrix = addNewRow(transposedMultipicationMatrix);

            close(transposedMultiplicationPipe[0]);
            write(transposedMultiplicationPipe[1], &transposedMultipicationMatrix.rows, sizeof(int));
            write(transposedMultiplicationPipe[1], &transposedMultipicationMatrix.columns, sizeof(int));
            write(transposedMultiplicationPipe[1], transposedMultipicationMatrix.arr,
                (size_t)transposedMultipicationMatrix.rows * transposedMultipicationMatrix.columns * sizeof(int));
            close(transposedMultiplicationPipe[1]);

            freeAllocatedMemory(&transposedMultipicationMatrix);
        ...
        if( pid2 == 0 ){
            ...
            close(transposedMultiplicationPipe[1]);
            read(transposedMultiplicationPipe[0], &transposeResult.rows, sizeof(int));
            read(transposedMultiplicationPipe[0], &transposeResult.columns, sizeof(int));
            transposeResult = createMatrix(transposeResult.rows, transposeResult.columns);
            read(transposedMultiplicationPipe[0], transposeResult.arr,(size_t)transposeResult.rows * transposeResult.columns * sizeof(int));
            close(transposedMultiplicationPipe[0]);

            printf("\nTransposed multiplication result with a new random row:\n");
